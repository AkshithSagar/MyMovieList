...
export class SuggestionsComponent implements OnInit {
  ...
  diplaymovies(){
    this.getapi.getTopFiveMovies().subscribe((results)=>{
      ...
      // }
      this.suggestions.push(new Suggestions(JSON.stringify(results[0]["Name"]),JSON.stringify(results[0]["Genre"]), JSON.stringify(results[0]["Review"])  , 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTy99-JIoQD_tDXwVlktYQVhmWUZPAVX92Tq4FIssNpJ9WDQYrP'))
      this.suggestions.push(new Suggestions(JSON.stringify(results[1]["Name"]),JSON.stringify(results[1]["Genre"]), JSON.stringify(results[1]["Review"])  , 'https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcS5uxkfEsqCe-oN5gfCK0jE2FiXlntNoYuyNmookn5PBBMTV5Yj'))
      this.suggestions.push(new Suggestions(JSON.stringify(results[2]["Name"]),JSON.stringify(results[2]["Genre"]), JSON.stringify(results[2]["Review"])  , 'https://m.media-amazon.com/images/I/91BT--NUiKL._AC_UY436_FMwebp_QL65_.jpg'))
      this.suggestions.push(new Suggestions(JSON.stringify(results[3]["Name"]),JSON.stringify(results[3]["Genre"]), JSON.stringify(results[3]["Review"])  , 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTy99-JIoQD_tDXwVlktYQVhmWUZPAVX92Tq4FIssNpJ9WDQYrP'))
      this.suggestions.push(new Suggestions(JSON.stringify(results[4]["Name"]),JSON.stringify(results[4]["Genre"]), JSON.stringify(results[4]["Review"])  , 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZhbU5pWXmpsBxgV6PFL-kAxVAihDFPueGh0n2HoUbWZdFp_AC'))
